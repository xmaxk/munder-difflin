...
export interface AgentMeta {
  ...
  sandbox?: boolean;
  /** Worker capability profile. 'lightweight' = a minimal, cwd-confined worker for
   *  WEAK/LOCAL models: the heavy hive protocol seed is stripped and external paths
   *  (its own agent dir included) are DENIED at the tool layer, so a small model
   *  spends its limited turn on the deliverable instead of inbox/memory housekeeping
   *  (a reproduced local-30B failure). It stays a real worker — the idle-done plugin
   *  + wall-clock reaper still signal completion and god still verifies its files —
...
export class HiveManager {
  ...
  private injectedPrompt(
    ...
    // Lightweight (weak/local) worker: a MINIMAL seed. The full hive protocol —
    // memory.md/inbox maintenance, outbox schema, KG/palace, Slack replies — consumes
    // a small model's turn before it ever reaches the task (reproduced live: a local
    // 30B did agent-dir housekeeping and never wrote the deliverable). Its agent dir
    // is denied at the tool layer anyway (opencode external_directory), and its done
    ...
    const spawnQueueLine = meta.isGod && this.orchestratorMaySpawn()
      ? `SPAWNING A WORKER: you can start an ephemeral worker yourself by writing ONE JSON file into ${inRoot('spawn-requests')}/<id>.json. Required: \`objective\` (what the worker must do) and \`cwd\` (the repo it runs in). Optional: \`name\`, \`command\`, \`provider\`, \`model\`, \`isolate\` (default true = its own git worktree), \`tokenCap\`, \`slack\` ({channel, thread_ts}) to route its failures back to a thread, and \`profile\`. Set \`"profile":"lightweight"\` when you judge a worker's model too WEAK to carry the full hive protocol — it strips the protocol seed and steers the worker's file tools to its cwd (its agent dir is denied), so a small model spends its turn writing the deliverable instead of doing inbox/memory housekeeping. This is a CAPABILITY judgment, not a rule about where the model runs: small local models (e.g. a local 30B-class coder) are the common case and a proven one to reach for, but decide by capability — if a full-citizen worker already stalled without producing a deliverable, retry it lightweight regardless of engine. A lightweight worker does not self-report; the idle-done plugin + reaper release it and YOU verify its files and close any Slack loop. Leave \`profile\` unset (full hive citizen) for capable models (claude, codex, gpt, gemini, and other strong cloud engines). The harness polls that directory, spawns \`worker-<id>\`, and moves the request to \`spawn-requests/.done/\` on success or \`.failed/\` with a reason. This is the ONLY way you can spawn; a hire manifest under research/hires/ needs the human to confirm it in the UI, so it is not a route you can complete on your own. Reuse an existing agent first, as above — a worker is a fresh spend every time. WORKER FAILURE HANDLING: when a worker finishes but its deliverable is MISSING, or its done is labeled "deliverables UNVERIFIED" (the honest opencode idle-done: the model stopped without producing output) — do NOT re-spawn it with the IDENTICAL objective+model, which reliably reproduces the same failure. On the retry, CHANGE something: (a) SWAP TO A PROVEN CLOUD ENGINE (claude, codex, or an opencode/openrouter model) — NOT another local model: local availability is unreliable (a bigger local model like gpt-oss-120b may fail to LOAD entirely — "llama-server failed to start" — and a cold local model can take ~2min to load, exceeding the agent's request timeout, so it silently produces nothing). Only retry on the SAME local model if you first confirm it is warm/serving. (b) RESPAWN IT AS LIGHTWEIGHT — set \`"profile":"lightweight"\` on the retry request (best for a weak/local model): the harness itself strips the hive protocol and denies its agent dir at the tool layer, so it can only write the deliverable to its cwd. This is the built-in, reliable form of "stop it doing inbox/permission housekeeping instead of the task" — prefer it over hand-editing the objective. After a worker fails the SAME way TWICE, STOP re-spawning: set its task "blocked" and push an ASK-ME humanQA entry (e.g. {"q":"local/Qwen3-30B produced no deliverable twice — likely a model-capability limit; swap to gpt-oss-120b, or drop it?","askedAt":"<iso>"}). Never loop re-hiring a worker that has hit a capability wall — change model/objective once, then escalate.`
      : '';
